...
use crate::treedstructure::IndexTrees;
use crate::treedstructure::FilteredIndexQuads;
use crate::RcQuad;
...
pub struct TreedDataset {
    /// Underlying trees that manipulates indexes
    index_trees: IndexTrees,
...
impl TreedDataset {
    ...
    pub fn new_anti(s: bool, p: bool, o: bool, g: bool) -> Self {
        Self {
            index_trees: IndexTrees::new_anti(s, p, o, g),
            term_index: TermIndexMapU::<u32, RcTermFactory>::default()
        }
    }
    ...
    fn quads_with_opt_spog<'s>(&'s self, indexes: [Option<u32>; 4]) -> DQuadSource<'s, Self> {
        let quads = self.index_trees.filter(indexes);
...
impl MutableDataset for TreedDataset {
        ...
        W: TermData,
    {
        ...
        let modified = self.index_trees.insert_by_index([si, pi, oi, gi]);
        if !modified {
        ...
        W: TermData,
    {
        ...
        if let (Some(si), Some(pi), Some(oi), Some(gi)) = (si, pi, oi, gi) {
            let modified = self.index_trees.delete_by_index([si, pi, oi, gi]);
            if modified {
                self.term_index.dec_ref(si);
                self.term_index.dec_ref(pi);
                self.term_index.dec_ref(oi);
...
#[cfg(test)]
sophia::test_dataset_impl!(test_treedataset, TreedDataset);
