...
fn task_query_g<F, G, R> (f: R, generator: F, query_num: usize) where
    R: io::BufRead,
    G: MutableDataset,
    F: Fn() -> G
{
    ...
    let (time_first, time_rest, c) = request(&g, &generator, query_num);

    eprintln!("matching triple: {}\n", c);

    let (time_second, time_rest_second, _) = request(&g, &generator, query_num);
